...
export const Header: React.FC<HeaderProps> = ({ onOpenMobileMenu }) => {
  ...
  const { demoMode, toggleDemoMode, profileName, profileEmail, profileAvatar } = useData();
        ...
        <div className="flex items-center gap-3 flex-1 max-w-md">
          ...
          <div className="relative w-full">
            ...
            <input
              ...
              className="w-full pl-9 pr-12 py-2 bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/60 rounded-xl text-xs sm:text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 focus:outline-hidden focus:ring-2 focus:ring-violet-500/30 transition-all"
            />
            <div className="absolute right-3 top-1/2 -translate-y-1/2 hidden sm:flex items-center gap-0.5 px-1.5 py-0.5 rounded-md bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-[10px] font-mono font-medium text-slate-400 shadow-2xs">
              ⌘K
            </div>
          </div>
        </div>

        {/* Right Side: Demo Mode Switch, Theme Toggle, Notifications, Profile */}
        <div className="flex items-center gap-2 sm:gap-3">
          {/* Demo Data Switch Button */}
          <motion.button
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.96 }}
            onClick={toggleDemoMode}
            title={demoMode ? 'Switch to Empty State' : 'Load Sample Demo Data'}
            className={`hidden sm:inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-semibold transition-all cursor-pointer ${
              demoMode
                ? 'bg-amber-100 dark:bg-amber-950/60 text-amber-700 dark:text-amber-300 border border-amber-300 dark:border-amber-800'
                : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-slate-200/80'
            }`}
          >
            <Database className="w-3.5 h-3.5" />
            <span>{demoMode ? 'Demo Data: ON' : 'Demo Data: OFF'}</span>
          </motion.button>
          ...
          <div className="relative">
            ...
            >
              ...
              {unreadCount > 0 && (
                <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-violet-600 ring-2 ring-white dark:ring-slate-900 animate-pulse" />
              )}
            </motion.button>
              ...
              {notificationsOpen && (
                <>
                  ...
                  <motion.div
                    ...
                    className="absolute right-0 mt-2 w-72 sm:w-80 rounded-2xl bg-white dark:bg-slate-900 shadow-xl border border-slate-200 dark:border-slate-800 py-2 z-30"
                  >
              ...
              {profileOpen && (
                <>
                  ...
                  <motion.div
                    ...
                    exit={{ opacity: 0, scale: 0.95, y: -8 }}
                    transition={{ duration: 0.18, ease: [0.16, 1, 0.3, 1] }}
                    className="absolute right-0 mt-2 w-52 rounded-2xl bg-white dark:bg-slate-900 shadow-xl border border-slate-200 dark:border-slate-800 py-1.5 z-30"
                  >
                    ...
                    <div className="border-t border-slate-100 dark:border-slate-800 pt-1">
                      <button
                        ...
                        className="w-full text-left px-3.5 py-2 text-xs text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-950/40 flex items-center justify-between transition-colors cursor-pointer"
                      >
                        <span className="font-medium">Demo Data Mode</span>
                        {demoMode && <Check className="w-3.5 h-3.5" />}
                      </button>
                      <button
